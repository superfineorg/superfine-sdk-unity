...
@interface SuperfineTrackingHTTPClient : NSObject

@property (nonatomic, strong) SuperfineTrackingRequestFactory requestFactory;
@property (nonatomic, readonly) NSURLSession *session;
@property (nonatomic, weak)  id<NSURLSessionDelegate> httpSessionDelegate;

+ (SuperfineTrackingRequestFactory)defaultRequestFactory;

- (instancetype)initWithRequestFactory:(SuperfineTrackingRequestFactory _Nullable)requestFactory;
 ...
 */
- (NSURLSessionUploadTask *)upload:(JSON_DICT)batch host:(NSURL *_Nonnull)host credential:(NSString*_Nonnull)credential completionHandler:(void (^)(BOOL retry))completionHandler;
