...
namespace Superfine.Unity
{
    public class SuperfineSDKManagerStandalone : SuperfineSDKManagerBase
    {
        ...
        [DllImport(pluginName)]
        private static extern void SuperfineSDKManager_LogIAPBuyStart(IntPtr handle, string pack, double price, int amount, string currency);
        [DllImport(pluginName)]
        private static extern void SuperfineSDKManager_LogIAPBuyEnd(IntPtr handle, string pack, double price, int amount, string currency, int success);
        [DllImport(pluginName)]
        ...
        {
            if (managerHandle == IntPtr.Zero) return;
            SuperfineSDKManager_LogIAPBuyStart(managerHandle, pack, price, amount, currency);
        }

        public override void LogIAPBuyEnd(string pack, double price, int amount, string currency, bool isSuccess)
        {
            if (managerHandle == IntPtr.Zero) return;
            SuperfineSDKManager_LogIAPBuyEnd(managerHandle, pack, price, amount, currency, isSuccess ? 1 : 0);
        }
