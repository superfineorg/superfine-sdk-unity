...
- (void)logIAPBuy_pack:(const char*)pack price:(float)price amount:(int)amount currency:(const char*)currency transactionId:(const char*)transactionId receipt:(const char*)receipt
{
    if (![self assertInited]) return;
...
- (void)logIAPResult_pack:(const char*)pack price:(float)price amount:(int)amount currency:(const char*)currency isSuccess:(bool)isSuccess
{
    if (![self assertInited]) return;
