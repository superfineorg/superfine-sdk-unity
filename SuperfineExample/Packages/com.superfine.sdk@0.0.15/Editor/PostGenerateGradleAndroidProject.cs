    ...
    public class PostGenerateGradleAndroidProject : IPostGenerateGradleAndroidProject
    {
        ...
        public void OnPostGenerateGradleAndroidProject(string basePath)
        {
            ...
            if (activityNode != null)
            {
                ...
                AddQueryPackages(androidManifest, settings);
            }
        ...
        private bool DoesQueryPackageExist(XmlDocument manifest, string package)
        {
            var xpath = string.Format("/manifest/queries/package[@android:name='{0}']", package);
            return manifest.DocumentElement.SelectSingleNode(xpath, GetNamespaceManager(manifest)) != null;
        }

        private bool AddQueryPackage(XmlDocument manifest, string package)
        {
            if (DoesQueryPackageExist(manifest, package))
            {
                return false;
            }

            XmlNode queriesNode = FindChildNode(manifest, "queries");
            if (queriesNode == null)
            {
                queriesNode = manifest.CreateElement("queries");
                manifest.DocumentElement.AppendChild(queriesNode);
            }

            var element = manifest.CreateElement("package");
            AddAndroidNamespaceAttribute(manifest, "name", package, element);
            queriesNode.AppendChild(element);

            return true;
        }

        private bool AddQueryPackages(XmlDocument manifest, SuperfineSDKSettings settings)
        {
            if (manifest == null || settings == null) return false;

            var queryPackages = settings.androidQueryPackages;

            int numPackages = (queryPackages != null ? queryPackages.Length : 0);
            if (numPackages == 0) return false;

            var manifestHasChanged = false;
            for (int i = 0; i < numPackages; ++i)
            {
                manifestHasChanged |= AddQueryPackage(manifest, queryPackages[i]);
            }
        ...
        private bool AddPermissions(XmlDocument manifest, SuperfineSDKSettings settings)
        {
            ...
            if (settings.androidBluetoothConnect)
            {
                manifestHasChanged |= AddPermission(manifest, "android.permission.BLUETOOTH_CONNECT");
            }

            if (settings.androidGetAccounts)
            {
                manifestHasChanged |= AddPermission(manifest, "android.permission.GET_ACCOUNTS");
            }
            ...
            if (settings.androidPermissionQueryAllPackages)
            {
                manifestHasChanged |= AddPermission(manifest, "android.permission.QUERY_ALL_PACKAGES");
            }
            ...
            if (settings.androidPermissionJobService)
            {
                manifestHasChanged |= AddPermission(manifest, "android.permission.BIND_JOB_SERVICE");
            }
            ...
            if (settings.androidPostNotifications)
            {
                manifestHasChanged |= AddPermission(manifest, "android.permission.POST_NOTIFICATIONS");
            }

            if (settings.androidScheduleExactAlarm)
            {
                manifestHasChanged |= AddPermission(manifest, "android.permission.SCHEDULE_EXACT_ALARM");
            }

            if (settings.androidRequestIgnoreBatteryOptimizations)
            {
                manifestHasChanged |= AddPermission(manifest, "android.permission.REQUEST_IGNORE_BATTERY_OPTIMIZATIONS");
            }
