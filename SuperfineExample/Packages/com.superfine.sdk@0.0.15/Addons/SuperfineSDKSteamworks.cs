...
namespace Superfine.Unity
{
    public static class SuperfineSDKSteamworks
    {
        ...
        {
            SuperfineSDK.OnCreateInstance += () =>
            {
                SuperfineSDK.Standalone.SetSteamDRMCheck(SteamDRMCheck);
#if !UNITY_EDITOR && UNITY_STANDALONE_OSX
                SuperfineSDK.Standalone.SetSaveSteamActivationLink(true);
#endif
            };

            SuperfineSDK.OnInitializeInstance += () =>
            {
                int error = SuperfineSDK.Standalone.GetSteamDRMError();
                if (error > 0)
                {
                    Application.Quit();
                }
