...
- (void)configurationWithParams:(const char *)params
{
    ...
    {
        NSString* str = [data objectForKey:@"wrapperVersion"];
        if (str != nil)
        {
            config.wrapperVersion = str;
        }
    }
    ...
    {
        NSString* str = [data objectForKey:@"useSkanConversionSchema"];
        if (str != nil)
        {
            config.useSkanConversionSchema = [str boolValue];
        }
    }
    ...
    {
        NSString* str = [data objectForKey:@"enableCoppa"];
        if (str != nil)
        {
            config.enableCoppa = [str boolValue];
        }
    }
...
- (void)logIAPBuy_pack:(const char*)pack price:(double)price amount:(int)amount currency:(const char*)currency transactionId:(const char*)transactionId receipt:(const char*)receipt
{
    if (![self assertInited]) return;
    [[SuperfineSDKManager sharedInstance] logIAPBuy_pack:[SuperfineSDKUnityUtility stringFromCString:pack] price:price amount:amount currency:[SuperfineSDKUnityUtility stringFromCString:currency] transactionId:[SuperfineSDKUnityUtility stringFromCString:transactionId] receipt:[SuperfineSDKUnityUtility stringFromCString:receipt]];
}
...
- (void)logAdRevenue_network:(const char*)network revenue:(double)revenue currency:(const char*)currency mediation:(const char*)mediation networkData:(const char*)networkData
{
    if (![self assertInited]) return;
...
extern "C"
{
    ...
    void SuperfineSDKLogIAPBuy(const char* pack, double price, int amount, const char* currency, const char* transactionId, const char* receipt)
    {
        [[SuperfineSDKUnityInterface sharedInstance] logIAPBuy_pack:pack price:price amount:amount currency:currency transactionId:transactionId receipt:receipt];
    }
